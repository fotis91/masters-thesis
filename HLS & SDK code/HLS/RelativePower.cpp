 ...
 *
 * This HLS IP in C code, calculates the Relative Power   
 * for a single EEG channel as described in the thesis.
 * 
 *
 */ 
...
#define TotalBandStart 0
#define TotalBandEnd 21
...
//float unitcirclesegmentsarray[M] = {-3.141592741012,-1.570796370506,-0.785398185253,-0.392699092626,-0.196349546313,-0.098174773156,-0.049087386578};
...
void RelativePower(hls::stream<varvara>& a,float * RP){

	//internal variables initializations(temporary variables)
    int i=0,j=0,k=0,l=0,p=0;
	float x1[SIZE],x1i[SIZE],areadstream[SIZE/2];
	float magsmpax[SIZE/2],Phzx[SIZE/2];
	float totalbandsum=0,alphabandsum=0;
    varvara tmpa,tmprp;
	...
	//feature calculation
	for(k=TotalBandStart;k<=TotalBandEnd;k++){
        totalbandsum = totalbandsum + Phzx[k];
        if(k>=AlphaBandStart && k<=AlphaBandEnd)
        	alphabandsum = alphabandsum + Phzx[k];
	}


	//set the output, when the last signal goes high
	if(tmpa.last) *RP = (alphabandsum/totalbandsum)*100;
}
